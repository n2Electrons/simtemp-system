...
if [ -e "$DOCKER_ENV" ]; then
  KERNEL_IMAGE="/host-workspace/deployment/qemu/linux-imx-5.10/arch/arm/boot/zImage"
  DTB_FILE="/host-workspace/deployment/qemu/linux-imx-5.10/arch/arm/boot/dts/imx6q-sabresd.dtb"
  ROOTFS_IMAGE="/host-workspace/deployment/qemu/rootfs.cpio.gz"
else
